...
static int read_battery_life_extender(struct samsung_laptop *samsung)
{
	...
	if (commands->get_battery_life_extender == 0xFFFF){
		pr_info("failed to get battery life, abandon.");
    return -ENODEV;
  }
    ...
    pr_info("failed to get bat data.");
		return -ENODEV;
  }
...
static int read_usb_charge(struct samsung_laptop *samsung)
{
	...
	if (commands->get_usb_charge == 0xFFFF){
		pr_info("failed to get usb charge");
    return -ENODEV;
  }
	...
	if (data.data[0] != 0 && data.data[0] != 1){
    pr_info("failed read usb charge data");
		return -ENODEV;
  }
...
static int read_lid_handling(struct samsung_laptop *samsung)
{
    ...
    pr_info("failed to get lid handling.");
		return -ENODEV;
  }
...
static int kbd_backlight_enable(struct samsung_laptop *samsung)
{
    ...
    pr_info("failed to get kbd backlit");
		return -ENODEV;
  }
...
static int __init samsung_init(void)
{
    ...
    pr_info("DMI check failed.");
		return -ENODEV;
  }

  if ((!quirks->has_good_efi) && efi_enabled(EFI_BOOT)){
    pr_info("Won't load without a good EFI.");
    return -ENODEV;
  }
